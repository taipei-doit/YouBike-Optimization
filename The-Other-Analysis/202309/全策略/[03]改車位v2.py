...
for ym in yms:
    ...
    temp_col += [f'實際6點在站車_周間中位數{ym}']
    temp_col += [f'實際調度車數_周間中位數{ym}']
    temp_col += [f'建議初始在站車_周末中位數{ym}']
    temp_col += [f'實際6點在站車_周末中位數{ym}']
    temp_col += [f'實際調度車數_周末中位數{ym}']
...
# idle docker
is_idle_docker  = (dockerfree_clean['建議調整柱數_202309']<=reduce_threshould)

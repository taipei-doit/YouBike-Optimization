...
print('Start [04]extract_df_from_simulation.py.')
t = time.time()
...
print('Finished simulation_results_static_part, cost {time.time() - t} seconds.')
...
for stop_id, dates_data in results.items():
    for date, v in dates_data.items():
        ...
        # raise ValueError('test')
    # print(f'{stop_id} finished, cost {time.time() - t} seconds.')
...
# save
file_path = output_path + '/simulation_results_detail_part.csv'

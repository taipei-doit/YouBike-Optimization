...
def optimize_dispatch(today_dispatch, dispatch_capacity,
                      max_time_interval_hour=4, is_print=False):
    '將零散的調度盡可能少次，每次最大調度數為dispatch_capacity'
    dispatch_queue = today_dispatch
    log = []
    log_col = ['api_time', 'dis_suggest',
    ...
    if at_least_one_dispatch:
        ...
        for _, row in dispatch_queue.iterrows():
            # if row['api_time']==datetime.datetime(2023, 3, 6, 18, 8, 36):
                # break
            ...
            if is_overflow:  # 需補車已超過可操作柱
                ...
                dispatch_suggestion['dis_suggest'].append(dis_num)
                dispatch_suggestion['api_time'].append(api_time)
            temp_log.extend([is_change_sign, is_overflow,
...
def add_dispatch_for_afrenoon(dispatch_suggestion, current_dict,
                              morning_final_bikes, today_best_afternoon,
                              is_print=False):
...
def add_dispatch_for_tomorrow(dispatch_suggestion, current_dict, today, tomorrow,
                              is_print=False):
...
def fill_dispatch_na(today_data):
    ...
    y, m, d = today.split('-')
    # 為下午調度
    afternoon_end_time = datetime.datetime(int(y), int(m), int(d),
                                           afternoon_hour-1, 59, 59)
    ...
    if is_dis_for_afternoon.sum() > 0:
        ...
        today_data.loc[is_dis_for_afternoon, 'api_m6h_hour'] = afternoon_hour - 6
    # 下午、晚上調度一起調整
...
is_add_txn_when_empty_or_full = False
dispatch_capacity = 14  # 每次調度的最大車數
# 但有可能某站只有10車柱，因此實際調度車數 不可大於 (capacity - 2) = 10-2 = 8
...
for stop_id in set(data['stop_id']):
    ...
    capacity = next(iter(daily_data[stop_id].values()))
    for i in range(len(date_sequnce)):  # date loop
        ...
        dispatch_capacity = get_dispatch_capacity(dispatch_capacity, capacity, dispatch_reserve_number)
        # today_dispatch = pd.DataFrame(columns=('api_time', 'dispatch_num', 'available_bike'))
        dispatch_suggestion = optimize_dispatch(today_dispatch, dispatch_capacity)
        
        dispatch_suggestion = add_dispatch_for_afrenoon(dispatch_suggestion, current_dict, morning_final_bikes, today_best_afternoon)
        dispatch_suggestion = add_dispatch_for_tomorrow(dispatch_suggestion, current_dict, today, tomorrow)

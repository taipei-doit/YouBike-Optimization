...
def sampling_result(results, smaple_step=2):
    ...
    '''
    is_delta = (results['txn_on'] + results['txn_off'] + results['in'] + results['out']) != 0
...
idle_status['adjust_api_time'] = idle_status['data_time'] - pd.Timedelta(seconds=min_delay_secs)
# 
a = 0
results = []
...
for stop_id in stop_ids:
    ...
    idle_data = idle_data.merge(dispatch_count, how='left',
                                left_on='adjust_api_time',
                                right_on='time')
    idle_data = idle_data.drop(columns=['time_x', 'time_y', 'data_time'])
...
# save
make_sure_folder_exist(idle_path)

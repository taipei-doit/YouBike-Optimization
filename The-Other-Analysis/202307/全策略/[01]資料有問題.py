...
docker_adj_threshould = 5
yms = ref_yms + [valid_ym]
weekday_days = {'202303': 23, '202304': 20, '202307': 21}
weekend_days = {'202303': 8, '202304': 10, '202307': 10}
strategy_type = '缺車站點'
error_type1 = '異常_柱數變動'
...
# Load
# 實驗50站
test50 = pd.read_excel(dim_path+'/加強調度FINALv3+見車率(更新至112.06.26).xlsx',
...
unqualified['類型'] = None
# 正常時間過少
is_low_confidence = (unqualified[f'資料可信度_{valid_ym}'] < confidence_ratio_threshould)
unqualified.loc[is_low_confidence, '類型'] = strategy_type
# 柱數變動
docker_adj = (unqualified[f'柱數_{valid_ym}'] - unqualified[f'柱數_{ref_yms[0]}'])
is_docker_change = (docker_adj.abs()>=docker_adj_threshould)
unqualified.loc[is_docker_change, '類型'] = error_type1

...
def find_min_cum_txn_case(udata):
    '''
    (min_cum_txn 縮寫為 minct)
    定位並回傳交易淨值最小的交易淨值(min_cum_txn)、時間(minct_time)、在站車數(minct_available_bike)
    交易淨值最小 = 一天累積借車最多 = 理論上最需要車的時間
    '''
    min_accu_net_txn_index = udata['min_cum_txn'].idxmin()
    most_need_bike_row = udata.loc[min_accu_net_txn_index]
...
def find_max_cum_txn_case(udata):
    '''
    (max_cum_txn 縮寫為 maxct)
    定位並回傳交易淨值最大的交易淨值(max_cum_txn)、時間(maxct_time)、在站車數(maxct_available_bike)
    交易淨值最大 = 一天累積還車最多 = 理論上最需要車柱的時間
    '''
    max_accu_net_txn_index = udata['max_cum_txn'].idxmax()
    most_need_dock_row = udata.loc[max_accu_net_txn_index]
...
def find_empty_situation(udata):
    '''
    找到空車的狀況
    當該次API回傳可借車 <= warning_threshould即認定為無車
...
def find_full_situation(udata):
    '找到滿車的狀況，當該次API回傳(總柱數 - 可借車) <= warning_threshould即認定為無位'
...
for (date_m6h, stop_id), gdata in fdata.groupby(['date_m6h', 'stop_id']):
    ...
    # break
    # 0~6點是特殊時間，不列入空爆、淨值計算
    udata = gdata.loc[gdata['adjust_api_time'].dt.hour >= init_hour]
    if udata.shape[0] == 0:  # 當日除0~6以外完全無資料，以0~6當中最後一筆為代表
        udata = pd.DataFrame(gdata.iloc[-1:])
    ...
    # 交易淨值最小資訊
    minct_time, minct_available_bike = find_min_cum_txn_case(udata)
    # 交易淨值最大資訊
    maxct_time, maxct_available_bike = find_max_cum_txn_case(udata)
    # 當日缺車狀況
...
# save
agg.to_excel(output_path+'/redundancy_bike.xlsx', sheet_name='redundancy_bike')
print('Finished [05]agg_by_date_by_stop.py')
del agg
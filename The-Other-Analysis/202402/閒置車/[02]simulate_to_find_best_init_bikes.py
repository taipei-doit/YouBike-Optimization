...
def simulat_init_bikes(gdata, simulate_bike_step=2, is_print=False):
    ...
    simulat_results = {}
    # 給定預設水位 & init
    for init_available_bike in range(local_reserve_number, safety_capacity+1, simulate_bike_step):
        available_bike = init_available_bike
        dispatch_queue = []
        dispatch_queue_col = ['api_time', 'dispatch_num', 'available_bike']
        log = []
        log_col = [
            'api_time', 'available_bike_before_dispatch',
        ...
        for _, target_row in gdata.iterrows():
            ...
            is_negative_overflow = available_bike < local_reserve_number
            ...
            if is_overflow:  # 溢出
                if is_positive_overflow: # 正溢出=還爆
                    ...
                    overflow = available_bike - local_reserve_number
                # 超出額抹去，並記錄超出額
...
def get_best_init_and_dispatch_queue(simulat_results):
    ...
    best_init = local_reserve_number
    for init_bike, v in simulat_results.items():
        ...
        if is_single_direction:
            ...
            min_dispatch_sum, best_init = _find_min_dis_and_init_bike(
                min_dispatch_sum, best_init, v, init_bike
            )
...
# Main
print('Start [02]simulate_to_find_best_init_bikes.py.')
t = time.time()
...
# 計算建議的調度與預設車數
# loop preload
output_file_path = idle_path+'/best_init_and_raw_dispatch_queue_by_stop_by_date.pkl'
is_run_before = os.path.exists(output_file_path)
if is_run_before:
...
for (date, stop_id), gdata in fdata.groupby(['date_m6h', 'stop_id']):
    ...
    if results.get(stop_id): # 跑過的不用跑
        if results[stop_id].get(date):
            continue
    ...
    if capacity <= (reserve_number + dispatch_reserve_number):
        local_reserve_number = 2
    else:
        local_reserve_number = reserve_number
    safety_capacity = (capacity - local_reserve_number)
    
    # 迴圈模擬，尋找最適初始車並記錄調度，上下午拆兩半
    is_morning = (
        (gdata['adjust_api_time'].dt.hour < afternoon_hour) 
        & (gdata['adjust_api_time'].dt.hour >= init_hour)
    )
    ...
    m_best_init, m_min_dispatch_sum, m_dispatch_queue = get_best_init_and_dispatch_queue(
        m_simulat_results
    )
    ...
    a_best_init, a_min_dispatch_sum, a_dispatch_queue = get_best_init_and_dispatch_queue(
        a_simulat_results
    )
    ...
    results[stop_id][date]['reserve_number'] = local_reserve_number

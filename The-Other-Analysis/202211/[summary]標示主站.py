...
stops_coexistence = pd.read_csv(root_path+file_path)
stops_coexistence.index = stops_coexistence['stop_id']
stops_coexistence = stops_coexistence.iloc[:, 1:]


# 主站 = 群內各站的交集 = 群內總共現數最高者
# 直接對寬鬆版做，因為寬鬆版是嚴格版的擴充
main_stops = {} # group: stop_id
for group, group_data in stops_label.groupby('label_loose'):
    # break
    # 非正常群跳過
...
is_isolate = (stops_label['label_loose']=='獨立站')
stops_label.loc[is_isolate, 'stop_role'] = '主站'
is_critical = (stops_label['label_loose']=='需關注站')
...
# save
file_path = '/DM/站點分群_寬鬆/[summary]clustering_results_with_main.csv'
